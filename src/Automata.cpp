...
void Automata::create(){
    ...
    */
    ...
    stateTypes[0] = StateType::Error; //Start State is Error State (not confuse con trap state)
    ...
    stateTypes[6] = StateType::Error; //Trap and error state
...
void Automata::addTransitionExcludingChars(int sourceState, int destinationState, const std::vector<char>& excludedChars) {
...
void Automata::addTransitionForSpecificChars(int sourceState, int destinationState, const std::vector<char>& specificChars) {
...
int Automata::makeTransition(int currentState, char input){
    ...
    return transitionTable[currentState][input];
}

bool Automata::isErrorState(int state){
    return getStateType(state) == StateType::Error;
}
...
std::string Automata::getStateTypeString(StateType state) {
    switch (state) {
